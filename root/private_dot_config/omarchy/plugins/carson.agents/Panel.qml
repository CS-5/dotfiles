...
Panel {
  ...
  // ---- Bar label -------------------------------------------------------
  //
  // The bar shows Claude Code's current 5-hour session window as a percentage
  // next to the glyph. It is pinned to Claude on purpose: the panel's selected
  // provider moves with h/l and middle-click, and a bar number whose meaning
  // silently changes under you is worse than no number.
  ...
  readonly property var claudeProvider: {
    ...
    for (var i = 0; i < list.length; i++)
      if (list[i].providerId === "claude") return list[i]
    return null
  }

  // limitWindows() normalizes both collectors onto the titles "Session" and
  // "Weekly", so this picks the rolling window rather than the 7-day one.
  readonly property var claudeSession: {
    var windows = root.limitWindows(root.claudeProvider)
    for (var i = 0; i < windows.length; i++)
      if (windows[i].title === "Session") return windows[i]
    return null
  }

  // percent arrives as 0..1 from the collector.
  readonly property int claudeSessionPercent: root.claudeSession
    ? Math.round(root.claudeSession.percent * 100) : -1

  readonly property string barLabel: {
    if (root.claudeSessionPercent < 0) return root.glyph
    var pct = root.claudeSessionPercent + "%"
    // A vertical bar has no room for the glyph and the number side by side.
    return root.vertical ? pct : root.glyph + "  " + pct
  }

  // Hovering the bar wants the numbers the label had no room for.
  readonly property string barTooltip: {
    var windows = root.limitWindows(root.claudeProvider)
    if (windows.length === 0) return "Claude Code"
    var parts = []
    for (var i = 0; i < windows.length; i++)
      parts.push(windows[i].title + " " + Math.round(windows[i].percent * 100) + "%")
